...
export function NotesClient() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setQuickSearchQuery(quickSearchInput.trim());
  ...
  const updateQuickSearchInput = useCallback((value: string) => {
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 lg:flex-1 lg:justify-end max-w-3xl w-full lg:w-auto">
          <div className="flex items-center gap-2 shrink-0 border-t border-border/10 pt-4 sm:border-t-0 sm:pt-0 w-full sm:w-auto justify-start">
            ...
            >
              ...
              <kbd className="hidden sm:inline-flex h-5 select-none items-center gap-0.5 rounded border border-border bg-muted px-1.5 font-mono text-[9px] font-medium text-muted-foreground/70">
      ...
      <AnimatePresence>
        {quickSearchOpen && (
          <QuickSearchModal
            ...
            debouncedQuery={quickSearchQuery}
            inputRef={quickSearchInputRef}
            query={quickSearchInput}
            results={quickSearchResults}
            totalCount={quickSearchItems.length}
            onActiveIndexChange={setQuickSearchActiveIndex}
            onClose={closeQuickSearch}
            onQueryChange={updateQuickSearchInput}
...
function QuickSearchModal({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      onActiveIndexChange((selectedIndex + 1) % results.length);
      return;
    }
  ...
  return (
    ...
    >
      <motion.button
        type="button"
        className="absolute inset-0 bg-background/50 backdrop-blur-sm cursor-default"
        ...
        <div className="flex h-14 items-center gap-2 border-b border-border/40 px-3">
          ...
          {isDebouncing ? (
            ...
            <button
              ...
              className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-steel/40"
            >
