...
function SortableTaskRow({
  ...
  return (
    <div
      ...
      className={`flex items-center gap-2 rounded-md border bg-background px-2 py-2 ${isDragging ? "opacity-60" : ""}`}
    >
      <button
        type="button"
        {...attributes}
...
function SortableApiRow({
  ...
  return (
    <div
      ref={setNodeRef}
      style={{ transform: CSS.Transform.toString(transform), transition }}
      className={`rounded-md border bg-background p-2 ${isDragging ? "opacity-60" : ""}`}
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="rounded bg-muted px-2 py-1 text-[10px] font-semibold">
          #{order}
        </span>
        <button
          onClick={onSelect}
        ...
        >
          <span
            className={`rounded px-2 py-1 text-xs font-semibold ${methodStyles[api.method] ?? methodStyles.GET}`}
          >
            {api.method}
          </span>
        ...
        <button
          onClick={onEdit}
          className="cursor-pointer rounded border px-2 py-1 text-xs hover:bg-accent"
        >
          Edit
        </button>
        <button
          onClick={onDelete}
          className="cursor-pointer rounded border px-2 py-1 text-xs text-destructive"
        >
          Delete
        </button>
      </div>
      <p className="mt-2 text-xs text-muted-foreground">
        {api.summary || "No description"}
      </p>
      <div className="mt-2 flex flex-wrap gap-1.5">
        {roles.map((role, index) => {
          const currentAccess =
            accessMap.get(`${role.id}:${api.id}`) ?? "denied";
          ...
          return (
            <button
              key={role.id}
              onClick={() =>
                onRoleAccess(role.id, isAllowed ? "denied" : "allowed")
              }
...
export default function DocsPage() {
  ...
  const [editingApiForm, setEditingApiForm] = useState({
    method: "GET",
    path: "",
    summary: "",
    requestExample: "",
  ...
  const [apiForms, setApiForms] = useState<
    Record<string, { method: string; path: string; summary: string }>
  >({});
  ...
  const [pendingDeleteApiId, setPendingDeleteApiId] = useState<string | null>(
    null
  );
  ...
  const activeTask =
    editingTaskItems.find((item) => item.id === activeTaskId) ?? null;
  const activeApi = apis.find((api) => api.id === activeApiId) ?? null;

  const accessMap = useMemo(
    () =>
      new Map(
        accessRows.map((row) => [`${row.roleId}:${row.apiId}`, row.access])
      ),
    [accessRows]
  );
  const roleTickets = useMemo<RoleTicket[]>(
    () =>
      roles.map((role, index) => ({
        roleId: role.id,
        ticket: `R${index + 1}`,
  ...
  const ticketByRoleId = useMemo(
    () => new Map(roleTickets.map((row) => [row.roleId, row.ticket])),
    [roleTickets]
  );
  ...
  const filteredApis = useMemo(() => {
    ...
    const base = apis.filter(
      (api) =>
        !keyword ||
        [api.method, api.path].join(" ").toLowerCase().includes(keyword)
    );
    if (!selectedRole || showAllApis) return base;
    return base.filter(
      (api) => accessMap.get(`${selectedRole.id}:${api.id}`) === "allowed"
    );
  ...
  const readJsonSafe = useCallback(
    async <T,>(response: Response): Promise<T | null> => {
  ...
  const loadDocs = useCallback(async () => {
    ...
    const [nextRoles, nextFeatures, nextApis, nextAccessRows] =
      await Promise.all([
    ...
    if (!rolesRes.ok || !featuresRes.ok || !apisRes.ok || !accessRes.ok)
      throw new Error("Không thể tải dữ liệu docs");
  ...
  useEffect(() => {
    ...
    void loadDocs()
      .then((payload) => {
        ...
        setAccessRows(payload.nextAccessRows ?? []);
        setSelectedRoleId(
          (current) => current || payload.nextRoles?.[0]?.id || ""
        );
      })
      .catch((error: unknown) => {
        if (active)
          setErrorMessage(
            error instanceof Error ? error.message : "Load docs failed"
          );
      });
    return () => {
      active = false;
  ...
  async function createRole() {
    ...
    if (!roleForm.summary.trim())
      return setRoleFormError("Role summary is required");
    const response = await fetch("/api/docs/roles", {
      method: "POST",
      cache: "no-store",
    ...
    if (!response.ok) {
      ...
      return setErrorMessage(payload?.error || "Create role failed");
    }
  ...
  async function deleteRole(id: string) {
    ...
    if (role && PROTECTED_ROLE_NAMES.has(role.name))
      return setToast("Default roles cannot be deleted");
    const response = await fetch(`/api/docs/roles/${id}`, {
      method: "DELETE",
      cache: "no-store",
    });
    if (!response.ok) {
      const payload = await readJsonSafe<{ error?: string }>(response);
      return setToast(payload?.error || "Delete role failed");
    }
    setSelectedRoleId("");
    await refreshFromServer();
  }
  ...
  async function updateRole(role: Role) {
    const response = await fetch(`/api/docs/roles/${role.id}`, {
      method: "PATCH",
      cache: "no-store",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        summary: role.summary,
        focusTasks: role.focusTasks,
      }),
    });
    if (!response.ok) {
      const payload = await readJsonSafe<{ error?: string }>(response);
      return setErrorMessage(payload?.error || "Update role failed");
    }
    setErrorMessage("");
  }
  ...
  async function createFeature() {
    ...
    const response = await fetch("/api/docs/features", {
      ...
      cache: "no-store",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: featureForm.trim() }),
    });
    if (!response.ok) {
      const payload = await readJsonSafe<{ error?: string }>(response);
      return setErrorMessage(payload?.error || "Create feature failed");
    }
    setFeatureForm("");
    await refreshFromServer();
  }
  ...
  async function createApi(featureId: string) {
    const form = apiForms[featureId] ?? {
      method: "GET",
      path: "",
      summary: "",
    };
    if (!form.path.trim()) return;
    const response = await fetch("/api/docs/apis", {
      method: "POST",
      cache: "no-store",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...form, featureId }),
    });
    if (!response.ok) {
      const payload = await readJsonSafe<{ error?: string }>(response);
      return setErrorMessage(payload?.error || "Create API failed");
    }
    setApiForms((prev) => ({
      ...prev,
      [featureId]: { method: "GET", path: "", summary: "" },
    }));
    await refreshFromServer();
  }

  async function deleteApi(id: string) {
    const response = await fetch(`/api/docs/apis/${id}`, {
      method: "DELETE",
      cache: "no-store",
    });
    if (!response.ok) {
      const payload = await readJsonSafe<{ error?: string }>(response);
      return setToast(payload?.error || "Delete API failed");
    }
  ...
  async function updateAccess(roleId: string, apiId: string, access: Access) {
    const response = await fetch("/api/docs/access", {
      method: "PUT",
      cache: "no-store",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ roleId, apiId, access }),
    });
    if (!response.ok) {
      const payload = await readJsonSafe<{ error?: string }>(response);
      return setErrorMessage(payload?.error || "Update access failed");
    }
    await refreshFromServer();
  }
  ...
  function startEditRole(role: Role) {
    ...
    const mapped = (role.focusTasks || "")
      .split("\n")
      .map((item) => item.trim())
      .filter(Boolean)
      .map((item, index) => ({
        id: `${role.id}-${index}-${item.replace(/^\[(x| )\]\s*/i, "") || "item"}`,
        text: item.replace(/^\[(x| )\]\s*/i, "").trim(),
        checked: item.toLowerCase().startsWith("[x]"),
      }));
    setEditingTaskItems(
      mapped.length
        ? mapped
        : [{ id: `${role.id}-0-new`, text: "", checked: false }]
    );
  }
  ...
  async function saveEditRole(role: Role) {
    ...
    const serializedTasks = editingTaskItems
      .map((item) =>
        item.text.trim()
  ...
  async function toggleViewTaskCheck(role: Role, itemIndex: number) {
    const parsed = (role.focusTasks || "")
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean)
      .map((line, index) => ({
        id: `${role.id}-view-${index}`,
        text: line.replace(/^\[(x| )\]\s*/i, "").trim(),
        checked: line.toLowerCase().startsWith("[x]"),
      }));
    if (!parsed[itemIndex]) return;
    parsed[itemIndex] = {
      ...parsed[itemIndex],
      checked: !parsed[itemIndex].checked,
    };
  ...
  function onApiDragStart(event: DragStartEvent) {
    setActiveApiId(String(event.active.id));
  }
  async function onApiDragEnd(featureId: string, event: DragEndEvent) {
    ...
    await fetch("/api/docs/apis/reorder", {
      method: "PUT",
      cache: "no-store",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <main className="mx-auto h-screen w-full max-w-7xl overflow-hidden px-4 py-4 md:px-8 md:py-6">
      <header className="mb-4 space-y-2">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground md:text-base">
              CRUD roles, features, APIs và mapping quyền trực tiếp bằng SQLite.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex items-center gap-1 rounded px-2 py-1 text-xs ${showAllApis ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
            >
              {showAllApis ? (
              ...
              {showAllApis
                ? "Mode: All APIs"
                : `Mode: ${selectedRole ? ticketByRoleId.get(selectedRole.id) : "Role"}`}
            </span>
            <button
              onClick={() => setShowAllApis((prev) => !prev)}
              className={`rounded-md border px-3 py-2 text-xs hover:bg-accent ${showAllApis ? "border-primary bg-primary/10 text-primary" : ""}`}
            >
              {showAllApis ? "Focus selected role" : "Show all APIs"}
            </button>
          </div>
        </div>
        {errorMessage ? (
      ...
      {toast ? (
        <button
          onClick={() => setToast("")}
          className="fixed right-4 top-4 z-[60] rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-left text-sm text-destructive shadow"
        >
          {toast}
        </button>
      ) : null}
      ...
      <div className="flex h-[calc(100vh-9.5rem)] min-h-0 w-full gap-4 overflow-hidden">
        <aside className="flex h-full min-h-0 w-96 shrink-0 flex-col rounded-xl border bg-card p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                aria-label="Open create role modal"
              >
          ...
          <Accordion
            ...
            className="min-h-0 flex-1 space-y-2 overflow-y-scroll px-1 [scrollbar-gutter:stable]"
          >
            {roles.map((role) => (
              <AccordionItem
                key={role.id}
                ...
                className={
                  role.id === selectedRoleId
                    ? `${getRoleColor(roleIndexById.get(role.id) ?? 0)} border`
                    : "border-border"
                }
              >
                <AccordionTrigger className="cursor-pointer px-4 py-3">
                  <span>
                    <span
                      className={`mb-1 inline-flex rounded border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${getRoleColor(roleIndexById.get(role.id) ?? 0)}`}
                    >
                      {ticketByRoleId.get(role.id)}
                    </span>
                ...
                <AccordionContent className="border-t bg-muted/20 px-4 py-3">
                  {editingRoleId === role.id ? (
                    <>
                      ...
                      >
                        <SortableContext
                          items={editingTaskItems.map((item) => item.id)}
                          strategy={verticalListSortingStrategy}
                        >
                          <div className="space-y-2">
                            {editingTaskItems.map((item, index) => (
                              <SortableTaskRow
                                key={item.id}
                                item={item}
                                index={index}
                        ...
                        <DragOverlay>
                          {activeTask ? (
                            <div className="flex items-center gap-2 rounded-md border bg-card px-2 py-2 shadow-lg">
                              <span className="text-xs text-muted-foreground">
                                ⋮⋮
                              </span>
                              <span className="flex-1 text-xs">
                                {activeTask.text || "Role có thể làm..."}
                              </span>
                              <input
                                type="checkbox"
                      ...
                      <div className="mt-3 flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          onClick={() => void saveEditRole(role)}
                        ...
                        <button
                          ...
                          className="rounded-md border px-2.5 py-1.5 text-xs hover:bg-accent disabled:opacity-60"
                        >
                          Cancel
                        </button>
                      </div>
                    ...
                    <>
                      <ul className="space-y-1.5 pl-1 text-xs leading-relaxed">
                        {(role.focusTasks || "Chưa có mô tả BA docs.")
                          .split("\n")
                          .map((item, index) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <input
                                  ...
                                  checked={isChecked}
                                  onChange={() =>
                                    void toggleViewTaskCheck(role, index)
                                  }
                                  className="h-3.5 w-3.5 cursor-pointer"
                                />
                              </li>
                      ...
                      <div className="mt-3 flex justify-end">
                        <button
                          ...
                          className="cursor-pointer rounded-md border px-2.5 py-1.5 text-xs hover:bg-accent"
                        >
                          Edit
                        </button>
                      </div>
                  ...
                  {!PROTECTED_ROLE_NAMES.has(role.name) ? (
                    <div className="mt-4">
                      <Tooltip
                        open={pendingDeleteRoleId === role.id}
                        onOpenChange={(open) =>
                          setPendingDeleteRoleId(open ? role.id : null)
                        }
                      >
                        <TooltipTrigger>
                          <button
                            onClick={() => setPendingDeleteRoleId(role.id)}
                            className="rounded-md border px-2.5 py-1.5 text-xs text-destructive hover:bg-destructive/10"
                          >
                            Delete role
                          </button>
                        </TooltipTrigger>
                        <TooltipContent className="!bg-card !text-foreground !border">
                          <div className="inline-flex items-center gap-2 text-xs">
                            ...
                            <button
                              ...
                              className="rounded border px-2 py-0.5 text-destructive"
                            >
                              Yes
                            </button>
                            <button
        ...
        <section className="flex h-full min-h-0 min-w-0 flex-1 flex-col rounded-xl border bg-card p-4">
            ...
            <div>
              ...
              <p className="text-sm">
                ...
                <span className="font-semibold">
                  {selectedRole
                    ? `${ticketByRoleId.get(selectedRole.id)} · ${selectedRole.name}`
                    : "No role"}
                </span>
              </p>
            </div>
          ...
          <div className="mb-3 space-y-2 rounded-lg border bg-background p-3">
            <div className="grid gap-2 md:grid-cols-[1fr_auto]">
              <input
                value={featureForm}
                onChange={(event) => setFeatureForm(event.target.value)}
            ...
            {featureCards.map((feature) => {
              ...
              return (
                <div key={feature.id} className="rounded-lg border p-3">
                  ...
                  <div className="mb-2 grid gap-2 md:grid-cols-[90px_1fr_1fr_auto]">
                    <select
                      value={apiForms[feature.id]?.method ?? "GET"}
                      onChange={(event) =>
                        setApiForms((prev) => ({
                          ...prev,
                          [feature.id]: {
                            ...(prev[feature.id] ?? { path: "", summary: "" }),
                    ...
                    >
                      {Object.keys(methodStyles).map((method) => (
                        <option key={method}>{method}</option>
                      ))}
                    </select>
                    <input
                      value={apiForms[feature.id]?.path ?? ""}
                      onChange={(event) =>
                        setApiForms((prev) => ({
                          ...prev,
                          [feature.id]: {
                            method: prev[feature.id]?.method ?? "GET",
                            summary: prev[feature.id]?.summary ?? "",
                    ...
                    />
                    <input
                      value={apiForms[feature.id]?.summary ?? ""}
                      onChange={(event) =>
                        setApiForms((prev) => ({
                          ...prev,
                          [feature.id]: {
                            method: prev[feature.id]?.method ?? "GET",
                            path: prev[feature.id]?.path ?? "",
                  ...
                  {items.length ? (
                    <DndContext
                      sensors={sensors}
                      collisionDetection={closestCenter}
                      onDragStart={onApiDragStart}
                      onDragEnd={(event) =>
                        void onApiDragEnd(feature.id, event)
                      }
                    >
                      <SortableContext
                        items={items.map((api) => api.id)}
                        strategy={verticalListSortingStrategy}
                      >
                        <div className="space-y-2">
                          {items.map((api, index) =>
                            editingApiId === api.id ? (
                              ...
                              >
                                <div className="grid gap-2 md:grid-cols-[90px_1fr_1fr_auto_auto]">
                                  <select
                                    value={editingApiForm.method}
                                    onChange={(event) =>
                                      setEditingApiForm((prev) => ({
                                        ...prev,
                                        method: event.target.value,
                                      }))
                                    }
                                ...
                                <div className="mt-2 grid gap-2 md:grid-cols-2">
                                  <textarea
                                    value={editingApiForm.requestExample}
                                    onChange={(event) =>
                                      setEditingApiForm((prev) => ({
                                        ...prev,
                                        requestExample: event.target.value,
                                      }))
                                    }
                                    placeholder="Request example (JSON)"
                                    className="min-h-24 rounded-md border bg-card px-3 py-2 font-mono text-xs"
                                  />
                                  <textarea
                                    value={editingApiForm.responseExample}
                                    onChange={(event) =>
                                      setEditingApiForm((prev) => ({
                                        ...prev,
                              ...
                              <div key={api.id} className="space-y-1">
                                <SortableApiRow
                                  ...
                                  roles={roles}
                                  accessMap={accessMap}
                                  onSelect={() => setSelectedApiId(api.id)}
                                  onDelete={() => setPendingDeleteApiId(api.id)}
                                  onEdit={() => startEditApi(api)}
                                  onRoleAccess={(roleId, access) =>
                                    updateAccess(roleId, api.id, access)
                                  }
                                />
                                {pendingDeleteApiId === api.id ? (
                                  <Tooltip
                                    open
                                    onOpenChange={(open) => {
                                      if (!open) setPendingDeleteApiId(null);
                                    }}
                                  >
                                    <TooltipTrigger>
                                      <span className="ml-8 inline-block h-0 w-0" />
                                    </TooltipTrigger>
                                    <TooltipContent className="!bg-card !text-foreground !border">
                                      <div className="inline-flex items-center gap-2 text-xs">
                      ...
                      <DragOverlay>
                        {activeApi ? (
                          <div className="rounded-md border bg-card p-2 shadow-lg">
                            <span
                              className={`mr-2 rounded px-2 py-1 text-xs font-semibold ${methodStyles[activeApi.method] ?? methodStyles.GET}`}
                            >
                              {activeApi.method}
                            </span>
          ...
          {selectedApi
            ? (() => {
                const detail = getMethodDetail(selectedApi.method);
                return (
                  <div
                    className={`mt-3 rounded-lg border p-3 ${methodDetailBorderStyles[selectedApi.method] ?? "border-dashed"} ${methodDetailBackgroundStyles[selectedApi.method] ?? "bg-card"}`}
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {roles.map((role) => {
                        const access =
                          accessMap.get(`${role.id}:${selectedApi.id}`) ??
                          "denied";
                        const roleColor = getRoleColor(
                          roleIndexById.get(role.id) ?? 0
                        );
                        return (
                          <span
                            key={role.id}
                            className={`rounded border px-2 py-1 text-xs ${access === "allowed" ? roleColor : accessStyles[access]}`}
                          >
                            {role.name}: {access}
                          </span>
                        );
                        ...
                        <div>
                          ...
                          <pre className="overflow-x-auto rounded-md border bg-background p-2 text-xs">
                            {selectedApi.requestExample ||
                              detail.defaultRequest}
                          </pre>
                        </div>
                      ) : null}
                      <div>
                        <p className="mb-1 text-xs font-semibold text-muted-foreground">
                          {detail.responseTitle}
                        </p>
                        <pre className="overflow-x-auto rounded-md border bg-background p-2 text-xs">
                          {selectedApi.responseExample ||
                            detail.defaultResponse}
                        </pre>
                      </div>
                    </div>
                  </div>
        ...
        >
          ...
          >
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-base font-semibold">Add Role</h2>
                <p className="text-sm text-muted-foreground">
                  Tạo role mới và mô tả BA docs cho role đó.
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="space-y-3">
              <input
                ...
                placeholder="Role name, vd STAFF"
                className="w-full rounded-md border bg-background px-3 py-2 text-sm"
              />
              <input
              ...
              />
              <textarea
                value={roleForm.focusTasks}
                onChange={(event) =>
                  setRoleForm((prev) => ({
                    ...prev,
                    focusTasks: event.target.value,
                  }))
                }
                placeholder="BA docs: mỗi dòng là 1 việc role có thể làm"
