...
function StepLabel({
  ...
  return (
    <span
      className={`flex items-center gap-1.5 text-xs font-medium transition-all duration-300 ${isActive ? "text-foreground" : "text-muted-foreground"}`}
    >
      <span
        className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold transition-all duration-300 ${isActive ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
      >
        {step}
      </span>
      {text}
            ...
            <div className="relative px-6 pt-6 pb-5 flex items-start justify-between">
              <div>
                ...
                <h2 className="text-lg font-bold text-foreground flex items-center gap-2 mt-2">
                  <CurrentIcon
                    className={`w-5 h-5 ${isTroll ? "text-rose-500" : "text-primary"}`}
                  />
                  {stepTitle}
                </h2>
              </div>
              ...
              <button
                onClick={handleClose}
                className="mt-0.5 flex size-11 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground"
                aria-label="Đóng"
              >
                <X className="w-4 h-4" />
              </button>
